(function ($) {
  ...
  function setJobDetails(jobRow) {
    if (jobRow) {
      ...
      $(jobActiveShow).find('.job-text.last-update').text($(jobRow).data('cityname') || '');
      $(jobActiveShow).find('.job-text.last-update').text($(jobRow).data('jobcode') || '');
  ...
  function jobActiveShowHandler(el) {
    ...
    if (!$("tr#job-active-show").is(":visible") || $(jobActiveShow).prev("tr")[0] !== row[0]) {
      ...
      $(jobActiveShow).prev('tr').find('a.btn-table.download.close-details').hide().siblings('a').show(); 
